...
This example demonstrates running multiple coroutines concurrently using asyncio.gather(). 
Each coroutine represents an event that will be triggered after a delay.
"""
...
asyncio.gather() allows running all these events concurrently. 
All the events start at the same time, but they complete as their respective delays elapse.

"""


"""
Using an Event Loop with Timers
...
"""
Handling Asynchronous Events with asyncio.Queue()

An asyncio.Queue is a useful way to handle events asynchronously. 
In this example, one coroutine puts events into the queue, and another coroutine consumes those events.
...
"""
Using asyncio.Event to Trigger an Event

asyncio.Event() is a synchronization primitive that can be used to trigger 
an event from one coroutine that other coroutines wait on.
"""
...
asyncio.wait_for() imposes a 3-second timeout. 
If the task does not complete within this time, a TimeoutError is raised.
"""



"""
Simulating Event Streams with async for
This example demonstrates how you can simulate an event stream and process events 
as they occur using an asynchronous generator.
"""
...
def store_letters():
    words = ['alpha', 'beta', 'charlie']
    for word in words:
        yield f"{str.upper(word[0])} for {word}"
